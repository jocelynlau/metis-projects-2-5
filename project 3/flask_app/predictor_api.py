...
def make_prediction(feature_dict):
    ...
    """
    x_input = [
        float(feature_dict.get(name, 0)) for name in appt_model.feature_names
    ]

    pred_probs = appt_model.predict_proba([x_input]).flat

    probs = [{'name': appt_model.target_names[index], 'prob': pred_probs[index]}
